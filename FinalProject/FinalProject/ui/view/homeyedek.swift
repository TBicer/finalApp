...
class HomePage: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        _ = viewModel.dealSliderList
            .subscribe(onNext: { [weak self] deals in
                ...
                self?.startCarousel() // Carousel'ı başlat
            })
    }
    ...
    func startCarousel() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: true) { [weak self] _ in
            ...
            let currentIndex = strongSelf.dealCollectionView.indexPathsForVisibleItems.first?.item ?? 0
            let nextIndex = currentIndex + 1
            
            // Eğer son elemeye ulaştıysak, başa dön
            if nextIndex >= strongSelf.dealSliderList.count {
                // Başlangıç hücresine kaydır, animasyon ekle
                strongSelf.dealCollectionView.scrollToItem(at: IndexPath(item: 0, section: 0), at: .left, animated: true)
                // Zamanlayıcıyı yeniden başlat
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { // animasyon süresince bekleme
                    strongSelf.dealCollectionView.scrollToItem(at: IndexPath(item: 0, section: 0), at: .left, animated: false)
                }
            } else {
                // Sonraki elemeye kaydır
